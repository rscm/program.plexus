...
# sopstreams_builtin(name,iconimage,sop) -> This function processes the
# url received from sopstreams and does the magic for all *nix based OS's.
...
# osx_sopcast_downloader() -> Sopcast downloader thread to avoid curl bugs
# in OSX
...
def sopstreams(name, iconimage, sop):
    if not iconimage:
        iconimage = os.path.join(
            addonpath, 'resources', 'art', 'sopcast_logo.jpg')
    if "sop://" not in sop:
    ...
    if not xbmc.getCondVisibility('system.platform.windows'):
        ...
        if config == False:
            mensagemok(translate(30000), translate(30027),
                       translate(30028), translate(30029))
        else:
            import _winreg
            aReg = _winreg.ConnectRegistry(None, _winreg.HKEY_LOCAL_MACHINE)

            # Dirty hack to break sopcast h264 codec so double sound can be
            # avoided
            ...
            try:
                ...
                codec_file = os.path.join(os.path.join(
                    value.replace("SopCast.exe", "")), 'codec', 'sop.ocx')
                _winreg.CloseKey(aKey)
                if xbmcvfs.exists(codec_file):
                    xbmcvfs.rename(codec_file, os.path.join(os.path.join(
                        value.replace("SopCast.exe", "")), 'codec', 'sop.ocx.old'))
            except:
                pass
            aReg = _winreg.ConnectRegistry(None, _winreg.HKEY_LOCAL_MACHINE)
            aKey = _winreg.OpenKey(
            ...
            res = handle_wait_socket(int(settings.getSetting(
                'socket_time')), translate(30000), translate(30030))
            ...
            if res == True:
                ...
                handle_wait(int(settings.getSetting('stream_time')),
                            translate(30000), translate(30031), segunda='')
                try:
                ...
                if connected == True:
                    ...
                    listitem = xbmcgui.ListItem(
                        labelname, iconImage=iconimage, thumbnailImage=iconimage)
            ...
            for line in proc.stdout:
                print("result line" + line.rstrip())
                # dirty hack to break sopcast.exe player codec - renaming the
                # file later
            import _winreg
            aReg = _winreg.ConnectRegistry(None, _winreg.HKEY_LOCAL_MACHINE)
            try:
...
def sopstreams_builtin(name, iconimage, sop):
    try:
        ...
        if xbmc.getCondVisibility('System.Platform.Linux') and not xbmc.getCondVisibility('System.Platform.Android'):
            ...
            if os.uname()[4] == "armv6l" or os.uname()[4] == "armv7l" or settings.getSetting('openelecx86_64') == "true":
                if settings.getSetting('jynxbox_arm7') == "true":
                    cmd = [os.path.join(pastaperfil, 'sopcast', 'ld-linux.so.3'), '--library-path', os.path.join(pastaperfil, 'sopcast', 'libqemu'), os.path.join(pastaperfil, 'sopcast', 'qemu-i386'), os.path.join(
                        pastaperfil, 'sopcast', 'lib/ld-linux.so.2'), "--library-path", os.path.join(pastaperfil, 'sopcast', "lib"), os.path.join(pastaperfil, 'sopcast', 'sp-sc-auth'), sop, str(LOCAL_PORT), str(VIDEO_PORT)]
                else:
                    cmd = [os.path.join(pastaperfil, 'sopcast', 'qemu-i386'), os.path.join(pastaperfil, 'sopcast', 'lib/ld-linux.so.2'), "--library-path", os.path.join(
                        pastaperfil, 'sopcast', "lib"), os.path.join(pastaperfil, 'sopcast', 'sp-sc-auth'), sop, str(LOCAL_PORT), str(VIDEO_PORT)]
            elif settings.getSetting('openeleci386') == "true":
                cmd = [os.path.join(pastaperfil, 'sopcast', 'lib/ld-linux.so.2'), "--library-path", os.path.join(
                    pastaperfil, 'sopcast', "lib"), os.path.join(pastaperfil, 'sopcast', 'sp-sc-auth'), sop, str(LOCAL_PORT), str(VIDEO_PORT)]
            else:
                cmd = [os.path.join(pastaperfil, 'sopcast', 'ld-linux.so.2'), '--library-path', os.path.join(
                    pastaperfil, 'sopcast', 'lib'), os.path.join(pastaperfil, 'sopcast', SPSC_BINARY), sop, str(LOCAL_PORT), str(VIDEO_PORT)]

        elif xbmc.getCondVisibility('System.Platform.OSX'):
            cmd = [os.path.join(pastaperfil, 'sopcast', 'sp-sc-auth'),
                   str(sop), str(LOCAL_PORT), str(VIDEO_PORT)]

        elif xbmc.getCondVisibility('System.Platform.Android'):
            cmd = [str(settings.getSetting('android_sopclient')),
                   str(sop), str(LOCAL_PORT), str(VIDEO_PORT)]

        print(cmd)

        # Check if another instance of the sopcast executable might still be
        # running on the same port. Attempt to connect to server and video
        # ports giving the user the choice before creating a new subprocess
        try:
            sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        ...
        if existing_instance == True:
            ...
            if not option:
                if xbmc.getCondVisibility('System.Platform.Android') == "true":
                    ...
                    procshut = subprocess.Popen(
                        ['ps', '|', 'grep', 'sopclient'], shell=False, stdout=subprocess.PIPE)
                    ...
                    # openelec
                    os.system(
                        "kill $(ps aux | grep '[s]p-sc-auth' | awk '{print $1}')")
                    os.system(
                        "kill $(ps aux | grep '[s]p-sc-auth' | awk '{print $2}')")
                elif xbmc.getCondVisibility('System.Platform.OSX'):
        ...
        if settings.getSetting('debug_mode') == "false":
            spsc = subprocess.Popen(
                cmd, shell=False, bufsize=BUFER_SIZE, stdin=None, stdout=None, stderr=None)
        else:
            spsc = subprocess.Popen(cmd, shell=False, bufsize=BUFER_SIZE,
        ...
        ret = mensagemprogresso.create(
            translate(30000), "SopCast", translate(30035) % (str(20)))
        ...
        if res:
            ...
            if not xbmc.getCondVisibility('System.Platform.OSX'):
                ...
                xbmcplugin.setResolvedUrl(int(sys.argv[1]), True, listitem)
                player = streamplayer(
                    xbmc.PLAYER_CORE_AUTO, spsc_pid=spsc.pid, listitem=listitem)
                if int(sys.argv[1]) < 0:
                ...
                handle_wait(int(settings.getSetting('stream_time_osx')),
                            translate(30000), translate(30031), segunda='')
    ...
    def onPlayBackEnded(self):
        ...
        if os.path.exists("/proc/"+str(self.spsc_pid)) and xbmc.getCondVisibility("Window.IsActive(epg.xml)") and settings.getSetting('safe_stop') == "true":
            if not xbmc.Player(xbmc.PLAYER_CORE_AUTO).isPlaying():
                player = streamplayer(
                    xbmc.PLAYER_CORE_AUTO, spsc_pid=self.spsc_pid, listitem=self.listitem)
