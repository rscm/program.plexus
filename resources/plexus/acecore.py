...
#
# This file contains the acestream console of the addon. Initial versions
# were coded by Nouismons and so, this file is based on his work.
#
# Classes:
#
# Logger() -> Log class
...
def show_Msg(heading, message, times=3000, pics=addon_icon):
    try:
        ...
        print('[%s]: ShowMessage: Transcoding UTF-8 failed [%s]' %
              (addon_id, e), 2)
        try:
            xbmc.executebuiltin('XBMC.Notification("%s", "%s", %s, "%s")' % (
...
class _TSPlayer(xbmc.Player):
    ...
    def onPlayBackStarted(self):
        watcher_thread = threading.Thread(
            name='acestream_watcher', target=ace_control_thread).start()
...
class TSengine():
    ...
    def connect(self):
        ...
        i = 40
        while (i > 1):
            self.progress.update(0, translate(30046), translate(
                30047) + str('%s' % i) + ' ' + translate(30048))
            try:
                if pwin:
                    aceport = self.getWinPort()
                elif posx:
    ...
    def getosxPort(self):
        try:
            path = os.path.join('/Applications', 'Ace Stream.app', 'Contents', 'Resources',
                                'wineprefix', 'drive_c', 'users', 'IGHOR', 'Application Data', 'ACEStream', 'engine')
    ...
    def startosx(self):
        ...
        import subprocess
        comd = [os.path.join('/Applications', 'Ace Stream.app', 'Contents', 'Resources', 'Wine.bundle', 'Contents', 'Resources', 'bin', 'wine'), os.path.join(
            '/Applications', 'Ace Stream.app', 'Contents', 'Resources', 'wineprefix', 'drive_c', 'users', 'IGHOR', 'Application Data', 'ACEStream', 'engine', 'ace_engine.exe')]
        print comd
        try:
            ...
            try:
                if settings.getSetting('engine_app') == "1":
                    ...
                    command = ["sh", "/data/data/"+settings.getSetting(
                        'app_id')+"/files/program.plexus/org.acestream.engine/files/droidace.sh", settings.getSetting('app_id')]
                    if settings.getSetting('total_max_download_rate') != "0":
                        command.append('--download-limit')
                        command.append(settings.getSetting(
                ...
                self.progress.update(
                    0, "Acestreamengine.apk not installed", "")
        else:
            print("Linux not android..")
            if "arm" in os.uname()[4]:
                try:
                    command = ["sh", os.path.join(
                        pastaperfil, "acestream", "start_acestream.sh"), "--client-console"]
                    if settings.getSetting('total_max_download_rate') != "0":
                        command.append('--download-limit')
                        command.append(settings.getSetting(
                            'total_max_download_rate'))
                ...
                try:
                    command = ["sh", os.path.join(
                        pastaperfil, 'acestream', 'start.sh')]
                    if settings.getSetting('total_max_download_rate') != "0":
                        command.append('--download-limit')
                        command.append(settings.getSetting(
                            'total_max_download_rate'))
                ...
                if settings.getSetting('ace_cmd') == "0":
                    ...
                    acebin = os.path.join(
                        pastaperfil, 'acestream', 'acestreamengine')
                    command = [acebin, '--client-console',
                               '--lib-path', acefolder]
                    if settings.getSetting('total_max_download_rate') != "0":
                        command.append('--download-limit')
                        command.append(settings.getSetting(
    ...
    def get_link(self, index=0, title='', icon='', thumb=''):
        ...
        for k, v in self.files.iteritems():
            if v == index:
                self.filename = urllib.unquote(k).replace(
                    '/', '_').replace('\\', '_')
        try:
        ...
        while not self.tsserv.got_url and not self.progress.iscanceled() and not self.tsserv.err:
            self.progress.update(int(self.tsserv.proc),
                                 self.tsserv.label, self.tsserv.line)
            xbmc.sleep(200)
        ...
        if self.tsserv.event and save:
            ...
            comm = 'SAVE %s path=%s' % (
                self.tsserv.event[0]+' '+self.tsserv.event[1], urllib.quote(self.filename))
            self.TSpush(comm)
    ...
    def play_url_ind(self, index=0, title='', icon='', thumb=''):
        ...
        if settings.getSetting('aceplay_type') == str(1):
            if ":6878/" in self.lnk:
                self.lnk = self.lnk.replace(
                    ":6878", ":" + settings.getSetting('playerport'))
        if self.progress:
            self.progress.close()
        item = xbmcgui.ListItem(
            title, iconImage="DefaultVideo.png", thumbnailImage=thumb)
    ...
    def loop(self):
        ...
        if self.player.isPlaying():
            if self.player.getTotalTime() > 0:
                cpos = int((1-(self.player.getTotalTime() -
                               self.player.getTime())/self.player.getTotalTime())*100)
            else:
                cpos = 0
            if cpos in pos:
                pos.remove(cpos)
                comm = 'PLAYBACK ' + \
                    self.player.link.replace('\r', '').replace(
                        '\n', '')+' %s' % cpos
                self.TSpush(comm)
    ...
    def load_torrent(self, torrent, mode, host=server_ip, port=aceport):
        ...
        if mode != 'PID':
            ...
            spons = ''
        comm = 'LOADASYNC ' + \
            str(random.randint(0, 0x7fffffff)) + ' '+mode+' ' + torrent + spons
        self.TSpush(comm)
        ...
        if self.file_count > 1:
            ...
            for list in flist['files']:
                self.files[urllib.unquote_plus(
                    urllib.quote(list[0]))] = list[1]
...
class TSServ(threading.Thread):
    ...
    def exec_com(self):
        ...
        if comm == 'HELLOTS':
            ...
            servip = settings.getSetting('ip_addr')
            self.got_url = self.last_received.split(
                ' ')[1].replace('127.0.0.1', servip)
    ...
    def showStats(self, params):
        ...
        if st == 'idle':
            ...
            self.line = 'Seeds:%s Download:%sKb/s' % (
                params.split(';')[8], params.split(';')[5])
            engine_data = {"action": str(translate(30059)), "percent": str(params.split(';')[1]) + "%", "download": str(params.split(';')[5]) + " Kb/s", "upload": str(params.split(';')[
                7]) + " Kb/s", "seeds": str(params.split(';')[8]), "total_download": str(int(params.split(';')[10])/(1024*1024))+'Mb', "total_upload": str(int(params.split(';')[12])/(1024*1024))+'Mb'}
            if settings.getSetting('debug_mode') == "true":
            ...
            engine_data = {"action": str(translate(30060)), "percent": str(params.split(';')[1]) + "%", "download": str(params.split(';')[3]) + " Kb/s", "upload": str(params.split(';')[
                5]) + " Kb/s", "seeds": str(params.split(';')[6]), "total_download": str(int(params.split(';')[8])/(1024*1024))+'Mb', "total_upload": str(int(params.split(';')[10])/(1024*1024))+'Mb'}
            if settings.getSetting('engine-status') == "true":
                try:
            ...
            engine_data = {"action": str(translate(30061)), "percent": str(params.split(';')[1]) + "%", "download": str(params.split(';')[5]) + " Kb/s", "upload": str(params.split(';')[
                7]) + " Kb/s", "seeds": str(params.split(';')[8]), "total_download": str(int(params.split(';')[10])/(1024*1024))+"Mb", "total_upload": str(int(params.split(';')[12])/(1024*1024))+"Mb"}
            if settings.getSetting('engine-status') == "true":
                try:
                    lat123.set_information(engine_data)
...
def stop_aceengine():
    if settings.getSetting('shutdown-engine') == 'true' and settings.getSetting('kill_type') == '0':
        if xbmc.getCondVisibility('system.platform.windows'):
            ...
            if "arm" in os.uname()[4]:
                os.system("sh "+os.path.join(pastaperfil,
                    ...
                    if 'arm' not in os.uname()[4]:
                        if settings.getSetting('acestream_cachefolder') == '':
                            acestream_cachefolder_file = os.path.join(
                                os.getenv("HOME"), '.ACEStream', 'cache', '.acestream_cache')
                        else:
                            acestream_cachefolder_file = settings.getSetting(
                        ...
                        if settings.getSetting('acestream_cachefolder') == '':
                            ...
                            acestream_cachefolder_file = settings.getSetting(
                                'acestream_cachefolder')
                    folder, cachefiles = xbmcvfs.listdir(
                        acestream_cachefolder_file)
                    for cachefile in cachefiles:
                        if cache_file in cachefile:
                            xbmcvfs.delete(os.path.join(
                                acestream_cachefolder_file, cachefile))
                except:
                    pass

        elif xbmc.getCondVisibility('system.platform.OSX'):
            try:
                kill_cmd = [os.path.join('/Applications', 'Ace Stream.app', 'Contents', 'Resources', 'Wine.bundle', 'Contents', 'Resources', 'bin', 'wine'), os.path.join(
                    '/Applications', 'Ace Stream.app', 'Contents', 'Resources', 'wineprefix', 'drive_c', 'windows', 'system', 'taskkill.exe'), '/f', '/im', 'ace_engine.exe']
            ...
            try:
                procshut_ace = subprocess.Popen(
                    ['ps', '|', 'grep', 'python'], shell=False, stdout=subprocess.PIPE)
            ...
            if settings.getSetting('save') != "true":
                try:
                    if settings.getSetting('acestream_cachefolder') != '':
                        dirs, cache_files = xbmcvfs.listdir(os.path.join(
                            settings.getSetting('acestream_cachefolder'), '.acestream_cache'))
                        print dirs, cache_files
                        for cache_file in cache_files:
                            xbmcvfs.delete(os.path.join(settings.getSetting(
                                'acestream_cachefolder'), '.acestream_cache', cache_file))
                    else:
                        acestream_cachefolder_file = os.path.join(
                            '/sdcard', '.ACEStream', 'cache', '.acestream_cache')
                        dirs, cache_files = xbmcvfs.listdir(
                            acestream_cachefolder_file)
                        for cache_file in cache_files:
                            xbmcvfs.delete(os.path.join(
                                acestream_cachefolder_file, cache_file))
                except:
                    pass
    else:
...
class OverlayText(object):
    ...
    def __init__(self):
        ...
        # main window
        self._background = xbmcgui.ControlImage(origin_x, origin_y, window_w, window_h, os.path.join(
            addonpath, "resources", "art", "background.png"))
        self._acestreamlogo = xbmcgui.ControlImage(origin_x + int(float(window_w)/11.3), origin_y + int(float(
            window_h)/14), acelogo_w, acelogo_h, os.path.join(addonpath, "resources", "art", "acestreamlogo.png"))
        self._supseparator = xbmcgui.ControlImage(origin_x, origin_y + int(float(
            viewport_h)/12.176), window_w-10, 1, os.path.join(addonpath, "resources", "art", "separator.png"))
        self._botseparator = xbmcgui.ControlImage(
            origin_x, origin_y + window_h - 30, window_w-10, 1, os.path.join(addonpath, "resources", "art", "separator.png"))
        self._title = xbmcgui.ControlLabel(origin_x+int(float(window_w)/3.4), origin_y + text_h,
                                           window_w - 140, text_h, str(translate(30062)), font=font_max, textColor='0xFFEB9E17')
        self._total_stats_label = xbmcgui.ControlLabel(origin_x+int(float(window_h)/1.72), origin_y + int(float(
            window_h)/1.6), int(float(window_w)/1.7), 20, str(translate(30063)), font=font_min, textColor='0xFFEB9E17')
        # labels
        self._action = xbmcgui.ControlLabel(origin_x + text_lat, origin_y + fst_setting, int(
            float(text_w)*1.6), text_h, str(translate(30064)) + '  N/A', font=font_min)
        self._download = xbmcgui.ControlLabel(origin_x + text_lat, origin_y + fst_setting + text_h, int(
            float(text_w)*1.6), text_h, str(translate(30065)) + '  N/A', font=font_min)
        self._upload = xbmcgui.ControlLabel(origin_x + text_lat, origin_y + fst_setting +
                                            2*text_h, text_w, text_h, str(translate(30066)) + '  N/A', font=font_min)
        self._seeds = xbmcgui.ControlLabel(origin_x + text_lat, origin_y + fst_setting +
                                           3*text_h, text_w, text_h, str(translate(30067)) + '  N/A', font=font_min)
        self._total_download = xbmcgui.ControlLabel(
            origin_x + text_lat, origin_y + fst_stat_setting, text_w, text_h, str(translate(30068)) + '  N/A', font=font_min)
        self._total_upload = xbmcgui.ControlLabel(
            origin_x + text_lat, origin_y + fst_stat_setting + text_h, text_w, text_h, str(translate(30069)) + '  N/A', font=font_min)
        self._percent_value = xbmcgui.ControlLabel(
            origin_x+int(float(window_h)/1.05), origin_y + fst_setting, text_w, text_h, 'N/A', font=font_min)
    ...
    def set_information(self, engine_data):
        if self.showing == True:
            self._action.setLabel(str(translate(30064)) +
                                  '  ' + engine_data["action"])
            self._percent_value.setLabel(engine_data["percent"])
            self._download.setLabel(
                str(translate(30065)) + '  ' + engine_data["download"])
            self._upload.setLabel(str(translate(30066)) +
                                  '  ' + engine_data["upload"])
            self._seeds.setLabel(str(translate(30067)) +
                                 '  ' + engine_data["seeds"])
            self._total_download.setLabel(
                str(translate(30068)) + '  ' + engine_data["total_download"])
            self._total_upload.setLabel(
                str(translate(30069)) + '  ' + engine_data["total_upload"])
        else:
            pass
