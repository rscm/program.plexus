...
# addDir(name,url,mode,iconimage,total,pasta,fan_art="%s/fanart.jpg"%settings.getAddonInfo("path"),parser=None,parserfunction=None)
# -> AddDir function used in the whole addon
...
def addDir(name, url, mode, iconimage, total, pasta, fan_art="%s/fanart.jpg" % settings.getAddonInfo("path")):
    ...
    u += "?url="+urllib.quote_plus(url)+"&mode="+str(mode)+"&name=" + \
        urllib.quote_plus(name)+"&iconimage="+urllib.quote_plus(iconimage)
    contextmen = []
    liz = xbmcgui.ListItem(
