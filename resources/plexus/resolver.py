...
# Functions:
#
# go_to_id(p2p_type) -> Receives the type of stream (sop or ace) and
# activates the keyboard to receive the next argument. This can be the sop
# id, the acestream hash or the sop url.
    ...
    if p2p_type == 'ace':
        ...
        if (keyb.isConfirmed()):
            ...
            if search == '':
                ...
                channel_id = search
                sop.sopstreams(translate(30021) + ' ( ' +
                               str(channel_id) + ')', '', str(channel_id))
