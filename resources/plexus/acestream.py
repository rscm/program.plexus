...
def load_local_torrent():
    ...
    if torrent_file:
        if xbmc.getCondVisibility('system.platform.windows'):
            acestreams("Local .torrent ("+str("file:\\" +
...
def acestreams(name, iconimage, chid):
    ...
    if settings.getSetting('engine_app') != '2' and settings.getSetting('engine_app') != '3':
        if settings.getSetting('aceplay_type') == "2":
            ...
            ret = pDialog.create(translate(30000), translate(
                30038), translate(30039), translate(30040))
            pDialog.update(0)
            xbmc.sleep(3000)
            pDialog.update(100)
            pDialog.close()
...
def acestreams_builtin(name, iconimage, chid):
    if xbmc.getCondVisibility('system.platform.windows'):
        try:
            ...
            subprocess.Popen("wmic process where ExecutablePath='" +
                             needed_value.replace('\\', '\\\\')+"' delete", shell=True)
            xbmc.sleep(200)
    ...
    if chid != '':
        ...
        if chid.find('http://') == -1 and chid.find('.torrent') == -1:
            ...
            out = TSPlayer.load_torrent(chid, 'TORRENT', port=aceport)
        if out == 'Ok':
            TSPlayer.play_url_ind(
                0, name + ' (' + chid + ')', iconimage, iconimage)
            TSPlayer.end()
            return
        else:
