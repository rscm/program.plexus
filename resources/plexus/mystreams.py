...
def my_streams_menu():
    ...
    addDir('[B][COLOR maroon]'+translate(30009)+'[/COLOR][/B]', MainURL,
           11, os.path.join(addonpath, art, 'plus-menu.png'), 1, False)
        ...
        if (keyb.isConfirmed()):
            ...
            if stream == '':
                ...
                if 'acestream://' not in stream and '.acelive' not in stream and 'sop://' not in stream:
                    ...
                    if yes:
                        iconimage = xbmcgui.Dialog().browse(1, translate(
                            30013), 'video', '.png|.jpg|.jpeg|.gif', True)
                    else:
                        if 'acestream://' in stream or '.acelive' in stream or '.torrent' in stream:
                            iconimage = os.path.join(
                                addonpath, 'resources', 'art', 'acestream-menu-item.png')
                        elif 'sop://' in stream:
                            iconimage = os.path.join(
                                addonpath, 'resources', 'art', 'sopcast-menu-item.png')
                        else:
                            iconimage = ''
                    # name
...
def remove_stream(name, url):
    ...
    try:
        os.remove(ficheiro)
        xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % (
            translate(30000), translate(30016), 1, os.path.join(addonpath, "icon.png")))
        xbmc.executebuiltin("Container.Refresh")
    except:
        pass
